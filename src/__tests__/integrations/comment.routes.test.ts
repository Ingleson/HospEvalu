...
const professionalData = {
    ...
    hospital_cnpj: "78.014.887/0001-65"
}
...
describe("Testando rotas comment", () => {
  ...
  test("POST /comment -> Deve ser capaz de criar um novo comentário", async () => {
    ...
    const createUser = await request(app).post("/user").send(userWithOutPasswordData);
    ...
    const hospital = await request(app).post("/hospital").send(hospitalData).set("Authorization", `Bearer ${tokenUser}`)

    const createProfessional = await request(app).post("/professional").send(professionalData);
  ...
  test("GET /comment/professional/:id -> Deve ser capaz de pegar todos os comentários de um profissional específico", async () => {
    ...
    const createHospital = await request(app).post("/hospital").send(hospitalData).set("Authorization", `Bearer ${tokenUser}`)
    
    const commentResponse = await request(app).get(`/comment/professional/${professionalId}`).set("Authorization", `Bearer ${tokenUser}`);
  ...
  test("GET /comment/professional/:id -> Não deve ser capaz de pegar todos os comentários de um profissional específico, sem token", async () => {
    ...
    const commentResponse = await request(app).get(`/comment/professional/${createProfessional.body.id}`);
  ...
  test("PATCH /comment/:id -> Deve ser capaz de atualizar um comentário", async () => {
    ...
    const responseComment = await request(app).patch(`/comment/${commentId}`).send({content: "Comentário editado"}).set("Authorization", `Bearer ${tokenUser}`);
  ...
  test("PATCH /comment/:id -> Não deve ser capaz de atualizar um comentário, sem ser ADM", async () => {
    ...
    const loginUser = await request(app).post("/login/user").send(userNotAdminData);

    const createProfessional = await request(app).post("/professional").send(professionalData);

    const commentToBeUpdated = await request(app).get(`/comment/professional/${createProfessional.body.id}`).set("Authorization", `Bearer ${loginUser.body.token}`);

    const responseComment = await request(app).patch(`/comment/${commentId}`).set("Authorization", `Bearer ${loginUser.body.token}`);
  ...
  test("DELETE /comment/:id -> Deve ser capaz de excluir um comentário", async () => {
    ...
    const responseComment = await request(app).delete(`/comment/${commentId}`).set("Authorization", `Bearer ${tokenUser}`);

    expect(responseComment.status).toBe(200);    
    expect(responseComment.body).toMatchObject({message: "Comentário deletado com sucesso"})
  ...
  test("DELETE /comment/:id -> Não deve ser capaz de excluir um comentário, sem ser ADM", async () => {
    ...
    const commentToBeDeleted = await request(app).get(`/comment/professional/${createProfessional.body.id}`).set("Authorization", `Bearer ${loginUser.body.token}`);

    const responseComment = await request(app).delete(`/comment/${commentToBeDeleted.body.comments}`).set("Authorization", `Bearer ${loginUser.body.token}`);
